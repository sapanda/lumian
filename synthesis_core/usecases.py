...
def get_transcript_summary(transcript_id: int, interviewee:str, repo: TranscriptRepository):
    ...
    new_text = '\n'.join(sentence_list)
    results = summarize_transcript(new_text, interviewee)
    final_results = []
    for item in results:
        final_results.append([item[0], []])
        for num in item[1]:
            i, j = mapping[num]
            final_results[-1][-1].append([i,j])
