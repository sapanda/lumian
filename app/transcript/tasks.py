...
def _process_chunks(tct: Transcript) -> Transcript:
    ...
    paragraph_groups = chunk_by_paragraph_groups(
        tct.transcript,
        tct.interviewee_names[0],  # Support only one interviewee for now
    )

    summaries = []
    for group in paragraph_groups:
        summaries.append(_get_summarized_chunk(group))
